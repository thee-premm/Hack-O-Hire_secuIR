...
def _src_path(*parts):
    return os.path.join(_SRC_DIR, *parts)
...
def test_model_files_exist():
    print("\n=== Test 1: Model files exist ===")
    lr_ok = check("models/lr_model.pkl exists", os.path.exists(_src_path('models', 'lr_model.pkl')))
    iso_ok = check("models/iso_model.pkl exists", os.path.exists(_src_path('models', 'iso_model.pkl')))
...
def test_models_loadable():
    ...
    try:
        with open(_src_path('models', 'lr_model.pkl'), "rb") as f:
    ...
    try:
        with open(_src_path('models', 'iso_model.pkl'), "rb") as f:
...
def test_real_event(pipeline):
    ...
    try:
        csv_path = os.path.join(_SRC_DIR, '..', 'data', 'synthetic_logs.csv')
        df = pd.read_csv(csv_path, nrows=20, parse_dates=["timestamp"])
